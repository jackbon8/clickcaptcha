...
class Clicaptcha
{
    public function create($text = '')
    {
        ...
        $imagePathArr = array('1.jpg', '2.jpg', '3.jpg');
        $imagePath = $config['backimagePath'].$imagePathArr[rand(0, count($imagePathArr) - 1)];
        $fontPath = $config['font'];
        $text = $text ? (is_array($text) ? $text : preg_split('/(?<!^)(?!$)/u', $text)) : $this->randChars();
        foreach ($text as $v) {
            $fontSize = rand(20, 30);
            //字符串文本框宽度和长度
        ...
        setcookie('clicaptcha_text', implode(',', $text),time() + 600, '/');
        //创建图片的实例
